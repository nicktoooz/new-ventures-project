  ...
  return (
    <>
      <Quote
        ...
        onClose={() => setQuoteVisible(!quoteVisible)}
      />
...
function Hero({
  ...
  return (
    <div>
      <div
        className={`h-[80vh] md:h-[30em] relative after:absolute after:inset-0 after:content-[''] after:bg-gradient-cover before:opacity-[0.5] before:absolute before:inset-0 before:content-[''] before:h-full before:w-full before:bg-commercial-clean before:bg-center grid place-items-center`}
      >
        <div className="hero-content z-10 text-white max-w-[60em] px-10">
          ...
          <p>
            Our team of expert cleaners is dedicated to delivering top-notch
            cleaning services to businesses in the area. We understand that each
            business has unique cleaning needs, which is why we offer
            customizable cleaning packages tailored to your specific
            requirements.
          </p>
          <div className={`flex gap-5 flex-col sm:flex-row mt-5`}>
            <button
              onClick={() => setQuoteVisible(true)}
              className={`p-2 max-w-40 w-full bg-[#2A2A27] transition-all rounded text-white`}
            >
              Get a Quote
            </button>
            <button
              onClick={() => (location.href = "tel:+61406780941")}
              className={`py-2 max-w-40 w-full bg-[#f1d7ac] transition-all rounded text-black`}
            >
              Call us Now
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col items-center py-10">
      <p className="text-center max-w-[50em] m-10">
        Our professional house cleaning services make it easy to enjoy a
        spotless home and a hassle-free life. To find out more about our
        extensive services or obtain a free, no-obligation quote, call us now!
      </p>
...
function ServiceCard({ file, text }: { file: string; text: string }) {
  ...
  return (
    <div
      className="max-w-80 h-40 grid w-full rounded-md relative before:absolute before:w-full before:h-full before:inset-0 before:bg-gradient-to-l from-[#ffffff40] from-10% to-[#00000063] bg-contain bg-no-repeat bg-center bg-gray-200 overflow-hidden"
